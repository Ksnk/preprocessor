...
Препроцессор поставляется с «исходным текстом», находящимся в каталоге src и является результатом выполнения этого текста. В результирующий файлы вставляются шапки с номером версии и описанием. Файл readme.txt форматируется в соответствии с упрощенным wiki.
...
`src/Build.xlm` – make-файл для Phing’а. Он описывает пару макрокоманд – init – собственно вызов препроцессора и property – определение параметров +  вызов утилиты SubWCRev для внедрения номера ревизии SVN в текст.
...
* для начала необходимо вставить в каждый файл "заготовку" шапки
        ...
        /**
         * Some word about file in project
         *
         * <%=point('hat','comment');
...
Обратите внимание, что после point(...); пропущены строки! Дело в том, что шапка занимает в получившемся файле несколько строк, так что сообщения об ошибках и варнингах окажутся смещенными на эти несколько строк в большую сторону. Чтобы это не раздражало - не помешает добавить несколько пустых строк.
...
Если некоторый файл не пришло еще время оформлять как плагин, а пользоваться им уже хочется, можно сделать так файл site.css
...
Теги препроцессора можно вставлять прямо в `/* ... */` комментариях
...
Во всех этих случаях препроцессор обнаружит строковые комментарии и выведет  текст на следующей строке
