...
(одна из причин такого переименования та, что Eclipse не любил показывать файлы с точкой в начале имени в списке файлов проекта, к тому же для локальной отладки файлов проекта встроенным сервером, такой файл может мешать нормальной работе.).
...
Сначала требуется разместить где-то этот текст. Его можно поместить в переменную прямо в файле конфигурации.
...
### <a name="toc17"/>Проект нуждается в небольшой тестирующей сборочке, однако делать для нее новый файл конфигурации не хочется.
...
### <a name="toc18"/>В конце файла хочется поместить текст лицензионного соглашения
